...
async fn main() -> web3::contract::Result<()> {
    ...
    let contract_addr: H160 = if addr.len() == 40 {
      ...
      let bytecode = include_str!("../compileFiles/ContractBytecode.bin").trim_end();
      // Deploying a contract
      let new_contract = Contract::deploy(web3.eth(), include_bytes!("../compileFiles/ContractAbi.json"))?
          .confirmations(0)
          .execute(
              bytecode,
              (
